...
get_patch_last_supported_ver() {
        ...
        if [ -z "$pcount" ]; then
                av_apps=$(java -jar "$cli_jar" list-versions -p "$patches_jar" -b 2>&1 | awk '/Package name:/ { printf "%s\x27%s\x27", sep, $NF; sep=", " } END { print "" }')
...
patch_apk() {
        ...
        local cmd="java -jar '$cli_jar' patch '$stock_input' --purge -o '$patched_apk' -p '$patches_jar' -b --keystore=ks.keystore \
...
build_rv() {
        ...
        local list_patches
        if ! list_patches=$(java -jar "$cli_jar" list-patches -p "$patches_jar" -b --filter-package-name="$pkg_name" --versions --packages 2>&1); then
